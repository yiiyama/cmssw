...
BeamHaloProducer::produce(edm::Event& event, edm::EventSetup const&)
{
  ...
  auto CMProduct{std::make_unique<edm::HepMCProduct>()};
  CMProduct->addHepMCData(genEvent.get());
  event.put(std::move(CMProduct));

  auto genEventInfo{std::make_unique<GenEventInfoProduct>(genEvent.release())};
